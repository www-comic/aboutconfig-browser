...
const Ci = {
    ...
    nsIPrefLocalizedString: 'AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa'
};
...
class PrefRow {
  ...
  get type() {
    return this.value.constructor.name === 'number' ? 'int' : this.value.constructor.name;
  }
    ...
    if (this.exists && !this.editing) {
      ...
      if (this.type == "String" && this.value.length > MAX_PLACEABLE_LENGTH) {
        ...
        let spanL10nId = this.hasUserValue
          ...
          outerSpan.innerText = this.value + (this.hasUserValue ? ' (custom)' : ' (default)');
      }
      outerSpan.appendChild(span);
      this.valueCell.textContent = "";
      this.valueCell.append(outerSpan);
      if (this.type == "Boolean") {
        this.editButton.title = 'Toggle';
      ...
      if (this.editing) {
        ...
        delete this.inputField;
        for (let type of ["boolean", "int", "string"]) {
          ...
          radioSpan.innerText = {boolean:'Boolean',int:'Integer',string:'String'}[type];
